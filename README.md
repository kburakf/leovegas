...
- [Prisma](https://www.prisma.io/) for database modelling, migration and type-safe access (Mys, MySQL & MongoDB)
...
- [Instructions](#instructions)
  ...
  - [Prisma Setup](#prisma-setup)
    ...
    - [2. MySQL with Docker](#2-MySQL-with-docker)
...
Setup a development MySQL with Docker. Copy [.env.example](./.env.example) and rename to `.env` - `cp .env.example .env` - which sets the required environments for MySQL such as `MYSQL_ROOT_PASSWORD` and `MYSQL_DB`. Update the variables as you wish and select a strong password.
...
+ DATABASE_URL=mysql://root:${MYSQL_ROOT_PASSWORD}@${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DB}
```
